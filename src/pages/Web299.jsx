...
export default function Web299() {
  const [form, setForm] = useState({ name: '', email: '', phone: '', message: '', consent: false, package: 'One-page vizitka — 299 €' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setForm({ name: '', email: '', phone: '', message: '', consent: false, package: 'One-page vizitka — 299 €' });
  ...
  const faqs = [
    {
      q: 'Je to jednorazová platba bez mesačných poplatkov?',
      a: 'Áno. Cena 299 € je jednorazová za kompletné zhotovenie webu. Web je po odovzdaní 100 % váš. Jediné ročné náklady sú za doménu a webhosting (cca 20–30 € ročne), ktoré platíte priamo poskytovateľovi — s ich výberom a nastavením vám kompletne pomôžeme.',
    },
    {
      q: 'Ako prebieha platba a kedy posielam peniaze?',
      a: 'Vyplníte formulár, my spravíme analýzu a do 24 hodín vám pošleme prvý vizuálny koncept zdarma. Ak poviete „áno, toto sa mi páči", uhradíte 50% zálohu a my web kompletne dokončíme. Zvyšných 50% platíte až vtedy, keď je web hotový, otestovaný a pripravený na spustenie.',
    },
    {
      q: 'Ako rýchlo je web hotový?',
      a: 'Štandardne do 5 pracovných dní od schválenia konceptu. Máme zabehnutý proces, vďaka ktorému eliminujeme zbytočné prestoje a dodáme web rýchlo aj bez kompromisov v kvalite.',
    },
    {
      q: 'Čo ak nemám žiadne texty ani fotografie?',
      a: 'Žiadny problém. Na základe krátkeho rozhovoru o vašom biznise napíšeme konverzné texty za vás. Pokiaľ nemáte vlastné fotky, vyberieme licencované profesionálne snímky z prémiových fotobánk, ktoré dokonale sadnú k vašej značke.',
    },
    {
      q: 'Je v cene SEO?',
      a: 'Áno — SEO základy sú v cene: správna štruktúra stránky, meta tagy, rýchlosť, mobilná verzia a nastavenie pre lokálne vyhľadávanie. Na pokročilé SEO (blog, linkbuilding, kľúčové slová) máme samostatnú službu v rámci balíka Biznis web.',
    },
    {
    ...
    {
      q: 'Čo ak sa mi prvý návrh nebude páčiť?',
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: C.white, color: C.text, minHeight: '100vh' }}>
          ...
          <div className="benefit-list" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', maxWidth: 560, margin: '0 auto 28px', textAlign: 'left' }}>
            {[
              ...
              <div key={b} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 14, color: C.textSub }}>
                <span style={{ color: C.blue, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>✓</span> {b}
              </div>
            ))}
          </div>
      ...
      <div style={{ background: C.white, padding: '64px 24px', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 36px)', fontWeight: 800, letterSpacing: -1, marginBottom: 12 }}>Stopercentná garancia spokojnosti</h2>
          <p style={{ fontSize: 16, color: C.textSub, lineHeight: 1.65, marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left', background: C.bg, borderRadius: 16, padding: '24px 28px' }}>
            {[
              ...
              <p key={p} style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{p}</p>
            ))}
          </div>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <div className="reviews-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <a href="https://maps.app.goo.gl/tDf7dRkYB2eZ3j9d7" target="_blank" rel="noopener noreferrer" className="card"
              style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer' }}>
      ...
      <div id="faq" style={{ background: C.white, padding: '64px 24px', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <p className="section-label" style={{ textAlign: 'center' }}>Odpovedáme</p>
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 36px)', fontWeight: 800, letterSpacing: -1, textAlign: 'center', marginBottom: 32 }}>Všetko, čo potrebujete vedieť pred štartom</h2>
          {faqs.map((item, i) => (
      ...
      <div style={{ background: C.blue, padding: '64px 24px' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 100, padding: '6px 18px', fontSize: 13, fontWeight: 600, color: '#fff', marginBottom: 20 }}>
            ⚡ Prijímame len 2 nové projekty týždenne pre udržanie prémiovej kvality
          </div>
          <h2 style={{ fontSize: 'clamp(24px, 4vw, 40px)', fontWeight: 800, color: '#fff', letterSpacing: -1.5, marginBottom: 14, lineHeight: 1.15 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={scrollToForm} style={{ display: 'inline-flex', alignItems: 'center', height: 56, padding: '0 32px', background: '#fff', color: C.blue, border: 'none', borderRadius: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Získať nezáväzný návrh riešenia →</button>
            <a href="https://wa.me/421907890600" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', height: 56, padding: '0 22px', background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1.5px solid rgba(255,255,255,0.4)', borderRadius: 14, fontSize: 15, fontWeight: 600, textDecoration: 'none', fontFamily: 'Inter, sans-serif' }}>💬 WhatsApp</a>
          </div>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', marginTop: 14 }}>prvý koncept do 24 hodín · úplne bez záväzkov · ukážeme vám, ako predávať viac</p>
        </div>
      </div>
      ...
      <div id="form299" style={{ padding: '64px 24px 96px', background: C.bg }}>
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          <p className="section-label" style={{ textAlign: 'center' }}>Rýchly dopyt</p>
          <h2 style={{ fontSize: 'clamp(20px, 3vw, 32px)', fontWeight: 800, letterSpacing: -1, textAlign: 'center', marginBottom: 8 }}>Získajte profesionálny návrh webu do 24 hodín zdarma</h2>
          <p style={{ textAlign: 'center', color: C.textSub, fontSize: 15, marginBottom: 28 }}>Napíšte nám základné detaily. Návrh je nezáväzný — ak sa vám nebude páčiť, nič neplatíte.</p>
          {formStatus === 'success' ? (
            <div className="card" style={{ padding: '48px 32px', textAlign: 'center' }}>
              <div style={{ fontSize: 48, marginBottom: 14 }}>🎉</div>
              <h3 style={{ fontSize: 24, fontWeight: 800, marginBottom: 10, color: C.blue }}>Skvelé, dáta dorazili!</h3>
      ...
      <footer style={{ background: '#111827', padding: '32px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', marginBottom: 4 }}>IČO: 56360495 · Trnava, Slovensko</p>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>© {new Date().getFullYear()} WebKlienti · Profesionálny webdizajn zameraný na výsledky</p>
      </footer>

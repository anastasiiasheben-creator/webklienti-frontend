...
function HomePage() {
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", background: '#f5f2eb', color: '#1a1a1a', minHeight: '100vh' }}>
      ...
      <main>
        <div style={{ background: '#1a1a1a', color: '#f5f2eb', padding: '72px 40px 88px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 720, margin: '0 auto', position: 'relative' }}>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 32 }}>
              {t.heroBadges.map(b => (<span key={b} className="trust-pill" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', padding: '6px 14px', borderRadius: 100, fontSize: 12, color: 'rgba(245,242,235,0.7)' }}>{b}</span>))}
            </div>
            ...
            <p style={{ fontSize: 17, color: 'rgba(245,242,235,0.65)', marginBottom: 36, lineHeight: 1.65, maxWidth: 520, margin: '0 auto 36px' }}>{t.heroSub}</p>
            <div className="hero-ctas" style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 48, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => scrollTo('reviews')} style={{ background: 'transparent', color: 'rgba(245,242,235,0.8)', border: '1.5px solid rgba(255,255,255,0.25)', padding: '17px 28px', borderRadius: 100, fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>{t.heroCtaSec}</button>
            </div>
            <div className="stats-row" style={{ display: 'flex', gap: 48, justifyContent: 'center', paddingTop: 36, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
              {[['80+', t.statClients], ['3×', t.statLeads], ['5 dní', t.statDays]].map(([val, label]) => (
                <div key={label}>
                  ...
                  <div style={{ fontSize: 12, color: 'rgba(245,242,235,0.6)', marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ padding: '64px 40px', background: '#f5f2eb' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'start' }} className="prob-sol-grid">
            ...
            <div style={{ background: '#1a1a1a', borderRadius: 24, padding: '36px 32px' }}>
              <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#ffd200', marginBottom: 10, fontWeight: 700 }}>{t.solutionLabel}</p>
        ...
        <div style={{ padding: '80px 40px', background: '#f5f2eb' }}>
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#555', marginBottom: 12, fontWeight: 700 }}>{t.casesLabel}</p>
                ...
                <div key={c.name} className="card-hover" style={{ background: '#1a1a1a', borderRadius: 24, padding: '40px 36px', color: '#f5f2eb' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
                    <div>
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(245,242,235,0.6)', letterSpacing: 1, textTransform: 'uppercase' }}>{c.tag}</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {[
                      { label: t.casesProblem, text: c.problem, color: '#e05c5c' },
                      ...
                      <div key={row.label} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                        ...
                        <span style={{ fontSize: 14, color: 'rgba(245,242,235,0.8)', lineHeight: 1.5 }}>{row.text}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div id="pricing" style={{ padding: '80px 40px', background: '#fff' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#555', marginBottom: 12, fontWeight: 700 }}>{t.pricingLabel}</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 48, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 14, color: '#555' }}>· {t.heroBadges[2]} · {t.heroBadges[3]}</span>
            </div>
        ...
        <div style={{ background: '#1a1a1a', padding: '80px 40px' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#999', marginBottom: 12, fontWeight: 700 }}>{t.howLabel}</p>
            ...
            <div className="process-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
              {t.steps.map((s, i) => (
                <div key={i} style={{ padding: '36px 28px', background: 'rgba(255,255,255,0.03)', borderRight: i < 3 ? '1px solid rgba(255,255,255,0.06)' : 'none', borderRadius: i === 0 ? '20px 0 0 20px' : i === 3 ? '0 20px 20px 0' : 0 }}>
                  ...
                  <p style={{ fontSize: 14, color: 'rgba(245,242,235,0.65)', lineHeight: 1.7 }}>{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div id="reviews" style={{ background: '#f5f2eb', padding: '80px 40px' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#555', marginBottom: 12, fontWeight: 700 }}>{t.reviewsLabel}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
              {staticReviews.map((r, i) => (
                <div key={i} className="card-hover" style={{ background: '#fff', border: '1.5px solid #eee', borderRadius: 20, padding: '32px 28px' }}>
                  <div style={{ color: '#b8860b', fontSize: 16, marginBottom: 14 }}>{'★'.repeat(r.rating)}</div>
                  <p style={{ fontSize: 15, color: '#333', lineHeight: 1.7, marginBottom: 24 }}>"{r.text}"</p>
                  <div>
                    <p style={{ fontWeight: 700, color: '#1a1a1a', fontSize: 14 }}>{r.author}</p>
                    {r.company && <p style={{ fontSize: 13, color: '#555' }}>{r.company}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 36 }}>
              {t.offerItems.map(item => (
                <div key={item} style={{ background: 'rgba(26,26,26,0.1)', borderRadius: 12, padding: '10px 20px', fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>✓ {item}</div>
              ))}
            </div>
        ...
        <div style={{ padding: '80px 40px', background: '#1a1a1a' }}>
          <div style={{ maxWidth: 700, margin: '0 auto' }}>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#999', marginBottom: 12, fontWeight: 700 }}>{t.faqLabel}</p>
            ...
            <div>
              {t.faqs.map((faq, i) => (
                <div key={i} className="faq-item">
                  ...
                  {openFaq === i && (
                    <p style={{ fontSize: 15, color: 'rgba(245,242,235,0.75)', lineHeight: 1.7, paddingBottom: 20, marginTop: -4 }}>{faq.a}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        <div style={{ background: '#f5f2eb', padding: '80px 40px', textAlign: 'center' }}>
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            ...
            <p style={{ color: '#555', marginBottom: 32, fontSize: 16 }}>Ozveme sa vám do 24 hodín. Bez záväzkov.</p>
        ...
        <div id="contact" style={{ padding: '80px 40px', maxWidth: 680, margin: '0 auto' }}>
          <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#555', marginBottom: 12, fontWeight: 700 }}>{t.formLabel}</p>
          <h2 style={{ fontSize: 'clamp(26px, 4vw, 44px)', fontWeight: 900, letterSpacing: -2, marginBottom: 12 }}>{t.formTitle}</h2>
          <p style={{ color: '#555', marginBottom: 40, fontSize: 16 }}>{t.formSub}</p>
          {formStatus === 'success' ? (
            <div style={{ background: '#1a1a1a', color: '#f5f2eb', borderRadius: 24, padding: '60px 40px', textAlign: 'center' }}>
              ...
              <p style={{ color: 'rgba(245,242,235,0.75)', fontSize: 16 }}>{t.successMsg}</p>
            ...
            <div style={{ background: '#fff', borderRadius: 24, padding: '44px 40px', display: 'flex', flexDirection: 'column', gap: 20, boxShadow: '0 4px 40px rgba(0,0,0,0.07)' }}>
              {t.fields.map(f => (
                <div key={f.key}>
                  <label htmlFor={`field-${f.key}`} style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: '#333' }}>{f.label}</label>
              ...
              <div>
                <label htmlFor="field-package" style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: '#333' }}>{t.packageLabel}</label>
              ...
              <div>
                <label htmlFor="field-message" style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: '#333' }}>{t.messageLabel}</label>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                ...
                <label htmlFor="field-consent" style={{ fontSize: 13, color: '#444', lineHeight: 1.5, cursor: 'pointer' }}>
                  {t.consentLabel}{' '}
              ...
              {formStatus === 'error' && <p style={{ color: '#c0392b', fontSize: 14 }} role="alert">{t.errorMsg}</p>}
              <button onClick={handleSubmit} disabled={formStatus === 'loading' || !form.consent} style={{ background: form.consent ? '#1a1a1a' : '#d0cec9', color: form.consent ? '#ffd200' : '#7a7773', border: 'none', padding: '16px', borderRadius: 100, fontSize: 16, fontWeight: 700, cursor: form.consent ? 'pointer' : 'not-allowed', opacity: formStatus === 'loading' ? 0.7 : 1, transition: 'background 0.2s, color 0.2s' }}>
      ...
      <footer id="footer" style={{ background: '#111', color: 'rgba(245,242,235,0.6)', textAlign: 'center', padding: '48px 40px', fontSize: 13 }}>
        ...
        <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
          <a href="mailto:info@webklienti.com" style={{ color: 'rgba(245,242,235,0.8)', textDecoration: 'none' }}>info@webklienti.com</a>
          <a href="tel:+421907890600" style={{ color: 'rgba(245,242,235,0.8)', textDecoration: 'none' }}>+421 907 890 600</a>
          <span style={{ color: 'rgba(245,242,235,0.6)' }}>webklienti.com</span>
        </div>
        <div style={{ marginBottom: 16, lineHeight: 1.9, color: 'rgba(245,242,235,0.6)' }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <button onClick={() => setPrivacyOpen(true)} style={{ background: 'none', border: 'none', color: 'rgba(245,242,235,0.6)', cursor: 'pointer', fontSize: 13, textDecoration: 'underline' }}>{t.privacyLink}</button>
        </div>
      ...
      {cookieVisible && (
        <div role="dialog" aria-label="Cookie súhlas" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 9999, background: '#1a1a1a', borderTop: '1px solid rgba(255,255,255,0.1)', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ color: 'rgba(245,242,235,0.9)', fontSize: 14, margin: 0, flex: 1 }}>
            🍪 {t.cookieMsg}{' '}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={declineCookies} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.4)', color: 'rgba(245,242,235,0.8)', padding: '8px 16px', borderRadius: 100, fontSize: 13, cursor: 'pointer' }}>{t.cookieDecline}</button>
      ...
      {privacyOpen && (
        <div role="dialog" aria-modal="true" aria-label={t.privacyTitle} style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '40px', maxWidth: 600, width: '100%', maxHeight: '80vh', overflowY: 'auto', position: 'relative' }}>
            <button onClick={() => setPrivacyOpen(false)} aria-label="Zavrieť" style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', color: '#555' }}>✕</button>
            <h2 style={{ fontWeight: 800, fontSize: 22, marginBottom: 24 }}>{t.privacyTitle}</h2>
            <pre style={{ whiteSpace: 'pre-wrap', fontSize: 14, lineHeight: 1.7, color: '#333', fontFamily: 'inherit' }}>{t.privacyContent}</pre>
          </div>
        </div>
      )}
    </div>
